...
class AuctionDetails extends Component {
  constructor(props) {
    ...
    const {  vehicleObj, negotiated , sellerprice } = this.props.route.params;
    ...
    this.state = {
     ...
     Textinput:true,
      ...
      carbodyimg:vehicleObj1.CarBodyImage,
...
////icon arraow /////////
  ...
  componentWillUnmount() {
    ...
    echo2.leaveChannel(`addseconds-channel`);
          // echo.leaveChannel('newAuctionChannel');
         echo2.leaveChannel('testing-channel');
  }
...
//  let h = diffInMs / (1000 * 60 * 60);
  ...
  Bidnow = () => {
    this.setState({loading:true})
    let mi = this.state.MinimumBid;
    let name = AppConstance.USER_INFO.USER_NAME;
    ...
    var value = new FormData();    
          ...
          if(this.state.bidvalue <= e){
            if(n === true){
              if(this.state.timeupdate === 1){
               ...
               value.append('negotiated', this.state.timechange);
        ...
        fetch(urlbid, {
          ...
          .then((responseJson) => {
            ...
            console.warn('check-bid-response :: ', responseJson);
            if (responseJson === true) {

              this.setState({yourcureentBid: this.state.bidvalue});
              this.setState({colorbidbtn: '#58D68D'});
              this.setState({loading: false});
               this.notification(this.state.bidvalue,a,name,this.state.item.Make);
  ...
  Bidnow3 = () => {
    ...
    if(userid === null || userid === ''){
      userid = AppConstance.USER_INFO.User_Id;
    }
    ...
    if (m >= m) {
        ...
        console.warn('userid' + userid + 'high' + b + 'auction' + a);
        ...
        fetch(urlbid, {
          ...
          .then((responseJson) => {
            ...
            if (responseJson === true) {
              console.warn('true--------'+responseJson);

              this.setState({yourcureentBid: this.state.bidvalue});
              this.setState({colorbidbtn: '#58D68D'});
              this.setState({loading: false});
 
              this.notification(m,a,name,this.state.item.Make)
              this.showSnackbarMessage();
              ...
              this.setState({loading: false});
              console.warn('false-------'+responseJson);
  ...
  bidapi = () => {
    ...
    fetch(vv, {
      ...
      .then((response) => response.json())
      .then((responseJson) => {
        console.warn('bidapi'+responseJson.data);
  ...
  notification =async(c,a,u,n)=>{

    let v = 'Current bid: '+c+', Auction id: '+a+', User Name: '+u+', Car Make: '+n;
  
    const FIREBASE_API_KEY = "AAAAeTW8SRw:APA91bFJG604bz0dEICq7QDjtaKTCbwKZkW--mCcGfnGiW5GxE9wdk40TdFfJ0Tm2FLyXi0hVIAEnfPj79vtwqHWwYUwAvXH5IXg2hq_GZul1aCWYIbv1p_hXB6NZsm5_WIZB9jzv5iS"
    const message = {
      registration_ids: [
'dqvNeKO1Sx-PTuGkJMbpYF:APA91bEVMW81GfCcEIQLD0xxyD-_bmZVXF04Z9nf6xDgZndfETozepKxBcqiNjI8Xs6NJOYBtb04C14oaUjNGwIK3s9tQgUSmZsXZUxBLOVhVsuiVwwiyWckRumjD9zmeG5zaVyOCe7X'
],
      notification: {
  ...
  componentDidMount() {
   ...
   Pusher .logToConsole = true;
    ...
    PusherClient2 = new Pusher('testappkey', {
      //cluster: 'mt1',
   ...
   echo2 = new Echo({
      broadcaster: 'pusher',
      client: PusherClient2,
    });

    echo2.channel('testing-channel').listen('testEvent', (e) => {
      let i = e.data.latestBid.toString();

      if (i !== this.state.yourbidcheck) {
        this.setState({colorbidbtn: 'orange'});
      }
      console.log('Pusher----Test bid at ' + e);
    ...
    echo2.channel('addseconds-channel').listen('addSeconds', (e) => {
      let i = e.data.latestBid.toString();

      if (i !== this.state.yourbidcheck) {
        this.setState({colorbidbtn: 'orange'});
      }
      console.log('Pusher----Test bid' + e);
...
<View
        ...
        <Modal
          transparent={true}
          visible={this.state.carbodyimage}
          onRequestClose={() => {
            console.log('close modal');
          }}>
          <View
            ...
            <View
              ...
              <TouchableOpacity
                onPress={() => this.setState({carbodyimage: false})}>
                <Image
                  style={{width: 30, height: 30}}
                  source={require('../Assets/icons/cancel.png')}
                />
            ...
            <View style={{paddingVertical: 10, paddingHorizontal: 5}}>
            <Image style={{   width:deviceWidth,height:450}}
   source={{ uri:'http://68.183.179.25/image/' + this.state.carbodyimg }}/>                   
            </View>
          </View>
        </Modal>
      ...
      ref = {(ref) => this.myRef=ref}
              initialSeconds = {this.state.Timer}
        onTimeOut = {()=>{ this.setState({complete: true});
                  
                  this.setState({Textinput:false});
                  this.showSnackbarMessageclosed();}}
        showSeparator = {true}
        // animateSeparator = {true}   
        separatorStyle={{color:'red',  fontSize:12,width:6,}}
...
{/* <TouchableOpacity onPress={()=> this.props.navigation.navigate('Auctions')} >
  <Text>ggg</Text>
</TouchableOpacity> */}
...
{this.state.feature === true ?
  <View>       
                  ...
                  <View
                    ...
                    <Text style={{fontSize: 13,fontWeight:'bold'}}>Seat Color</Text>
                    <Text style={{fontSize: 13,fontWeight:'bold'}}>
 ...
 */}
           ...
           : null }
            ...
            <Text style={{color:'green',fontSize:15 ,marginBottom:10, paddingHorizontal:24, marginTop:10}}>Note : Only fields written in Green are guarantee for accuracy</Text>
          </View>
        </ScrollView>
      </View>
    );
